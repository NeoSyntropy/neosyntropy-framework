...
class BackendClient:
    ...
    # ------------------------------------------------------------------
    # Concept registration — non-run observability objects
    # ------------------------------------------------------------------
    ...
    async def register_concept(
        ...
        POSTs to ``/observability/projects/{project_id}/{concept_type}`` and
        returns the server-assigned concept id, or ``None`` on any error.
        All failures are silently discarded to prevent concept registration from
        blocking application code.
        """
        path = f"/observability/projects/{project_id}/{concept_type}"
        response = await self._telemetry_post(path, {"manifest": manifest})
        if response is None:
            return None
        concept_id = response.get("id") or response.get("concept_id")
        return str(concept_id) if concept_id is not None else None

    async def concept_event(
        self,
        concept_id: str,
        event_type: str,
        payload: dict[str, Any],
        ...
        POSTs to ``/telemetry/concepts/{concept_id}/events``; failures are
        silently discarded.
        """
        await self._telemetry_post(
            f"/telemetry/concepts/{concept_id}/events",
