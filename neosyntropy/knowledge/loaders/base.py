...
class BaseLoader:
    ...
    RESERVED_METADATA_KEY = RESERVED_NEOSYNTROPY_KEY
    ...
    def _merge_metadata(
        ...
        reserved ``_neosyntropy`` key so that user PATCH updates cannot overwrite it.
        ...
        Returns:
            Merged metadata dictionary with provider fields under ``_neosyntropy``
        """
        # Strip any user-provided _neosyntropy — this key is reserved for the framework
        merged: Dict[str, Any] = strip_neosyntropy_metadata(user_metadata) or {}
        # Store provider metadata under reserved _neosyntropy key
        merged[RESERVED_NEOSYNTROPY_KEY] = dict(provider_metadata)
        return merged
