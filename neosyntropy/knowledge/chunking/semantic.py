...
from neosyntropy.knowledge.chunking.strategy import ChunkingStrategy
from neosyntropy.knowledge.document.base import Document
from neosyntropy.knowledge.embedder.base import Embedder
from neosyntropy.utils.log import log_debug


def _get_chonkie_embedder_wrapper(embedder: Embedder):
    """Create a wrapper that adapts NeoSyntropy Embedder to chonkie's BaseEmbeddings interface."""

    class _ChonkieEmbedderWrapper(BaseEmbeddings):
        """Wrapper to make NeoSyntropy Embedders compatible with chonkie."""

        def __init__(self, neosyntropy_embedder: Embedder):
            super().__init__()
            self._embedder = neosyntropy_embedder
...
class SemanticChunking(ChunkingStrategy):
    ...
    Args:
        embedder: The embedder to use for generating embeddings. Can be:
            ...
            - An NeoSyntropy Embedder (wrapped for chonkie compatibility)
    ...
    def __init__(
        ...
        if embedder is None:
            from neosyntropy.knowledge.embedder.openai import OpenAIEmbedder
    ...
    def _initialize_chunker(self):
        ...
        # - NeoSyntropy Embedder: wrap for chonkie compatibility
