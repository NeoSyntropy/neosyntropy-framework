...
from neosyntropy.knowledge.types import ContentType
from neosyntropy.utils.log import log_debug

RESERVED_NEOSYNTROPY_KEY = "_neosyntropy"
...
def merge_user_metadata(
    ...
    """Deep-merge two metadata dicts, preserving the ``_neosyntropy`` sub-key from both sides.

    Top-level keys from *incoming* overwrite those in *existing* (except ``_neosyntropy``).
    Keys inside ``_neosyntropy`` are merged individually so that info added
    ...
    for key, value in incoming.items():
        if key == RESERVED_NEOSYNTROPY_KEY:
            old_neosyntropy = merged.get(RESERVED_NEOSYNTROPY_KEY, {}) or {}
            new_neosyntropy = value if isinstance(value, dict) else {}
            merged[RESERVED_NEOSYNTROPY_KEY] = {**old_neosyntropy, **new_neosyntropy}
        else:
            merged[key] = value
    return merged


def set_neosyntropy_metadata(
    metadata: Optional[Dict[str, Any]],
    key: str,
    value: Any,
) -> Dict[str, Any]:
    """Set a key under the reserved ``_neosyntropy`` namespace in metadata."""
    if metadata is None:
        metadata = {}
    neosyntropy_meta = metadata.get(RESERVED_NEOSYNTROPY_KEY, {}) or {}
    neosyntropy_meta[key] = value
    metadata[RESERVED_NEOSYNTROPY_KEY] = neosyntropy_meta
    return metadata


def get_neosyntropy_metadata(
    metadata: Optional[Dict[str, Any]],
    key: str,
) -> Any:
    """Get a key from the reserved ``_neosyntropy`` namespace in metadata."""
    if not metadata:
        return None
    neosyntropy_meta = metadata.get(RESERVED_NEOSYNTROPY_KEY)
    if not isinstance(neosyntropy_meta, dict):
        return None
    return neosyntropy_meta.get(key)


def strip_neosyntropy_metadata(
    metadata: Optional[Dict[str, Any]],
) -> Optional[Dict[str, Any]]:
    """Return a copy of *metadata* without the reserved ``_neosyntropy`` key.
    ...
    if not metadata:
        return metadata
    return {k: v for k, v in metadata.items() if k != RESERVED_NEOSYNTROPY_KEY}


def _get_chunker_class(strategy_type):
    """Get the chunker class for a given strategy type without instantiation."""
    from neosyntropy.knowledge.chunking.strategy import ChunkingStrategyType
    ...
    strategy_class_mapping = {
        ChunkingStrategyType.AGENTIC_CHUNKER: lambda: _import_class(
            "neosyntropy.knowledge.chunking.agentic", "AgenticChunking"
        ),
        ChunkingStrategyType.CODE_CHUNKER: lambda: _import_class("neosyntropy.knowledge.chunking.code", "CodeChunking"),
        ChunkingStrategyType.DOCUMENT_CHUNKER: lambda: _import_class(
            "neosyntropy.knowledge.chunking.document", "DocumentChunking"
        ),
        ChunkingStrategyType.RECURSIVE_CHUNKER: lambda: _import_class(
            "neosyntropy.knowledge.chunking.recursive", "RecursiveChunking"
        ),
        ChunkingStrategyType.SEMANTIC_CHUNKER: lambda: _import_class(
            "neosyntropy.knowledge.chunking.semantic", "SemanticChunking"
        ),
        ChunkingStrategyType.FIXED_SIZE_CHUNKER: lambda: _import_class(
            "neosyntropy.knowledge.chunking.fixed", "FixedSizeChunking"
        ),
        ChunkingStrategyType.ROW_CHUNKER: lambda: _import_class("neosyntropy.knowledge.chunking.row", "RowChunking"),
        ChunkingStrategyType.MARKDOWN_CHUNKER: lambda: _import_class(
            "neosyntropy.knowledge.chunking.markdown", "MarkdownChunking"
        ),
    }
