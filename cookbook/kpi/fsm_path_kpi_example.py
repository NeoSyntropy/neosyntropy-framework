...
def build_fsm(provider: str) -> FSM:
    ...
    @functional_fsm_path_kpi(
        ...
        description=(
            "Compute a composite quality score (coverage × confidence) for the full FSM run."
        ),
    )
    def path_score(ctx: NodeContext) -> KpiResult:
        ...
        confidence = (
            float(answer_output.get("confidence", 0.0)) if isinstance(answer_output, dict) else 0.0
        )
        ...
        return KpiResult(
            ...
            score=score,
            reason=(
                f"coverage={coverage:.2f} confidence={confidence:.2f} path={path.nodes_executed}"
            ),
        )
