"""FSM cookbook: semantic routing with independent downstream branches.
...
def _client_for_example() -> Client:
    ...
    project = client.create_project(
        ...
        f"fsm-semantic-router-parallel-{stamp}",
        description="Live cookbook run for semantic routing branches",
    )
...
def billing_help(ctx):
    return ctx.result(
        output={"message": "Billing branch selected."},
        state_updates={"lane": "billing"},
        next_state="End",
    )


@node(id="ShippingHelp", input_schema=OpenInput, output_schema=TextOutput)
def shipping_help(ctx):
    return ctx.result(
        output={"message": "Shipping branch selected."},
        state_updates={"lane": "shipping"},
        next_state="End",
    )
...
def build_fsm(provider: str) -> FSM:
    """Build the parallel semantic-routing graph without performing I/O."""
    router = SemanticRouter(
        id="SupportIntent",
        input_schema=SupportRequest,
        routes={
            "billing": billing_help,
            "shipping": shipping_help,
        },
    ...
    return FSM(
        ...
        nodes=[capture_request, billing_help, shipping_help, out_of_scope],
        routers=[router],
        edges=[
            edge_deterministic("CaptureRequest", "SupportIntent"),
            edge_deterministic("BillingHelp", "End"),
            edge_deterministic("ShippingHelp", "End"),
...
def main() -> None:
    ...
    result = fsm.run(
        SupportRequest(text="I want help updating the billing email on my invoice."),
        state={},
        client=client,
    )
